...
#include <stdint.h>
#include <SPI.h>
#include "boblight_lamp.h"

#define NUM_OF_CHANNELS 24
...
void loop(void)
{
	...
	for (channel = 0; channel < NUM_OF_CHANNELS; channel++) {
		...
		values[channel] = Serial.read();
	}

	for (channel = 0; channel < (NUM_OF_CHANNELS / 3); channel++) {
		ShiftPWM.SetGroupOf3(channel, values[channel * 3],
				     values[channel * 3 + 1],
				     values[channel * 3 + 2]);
	}
