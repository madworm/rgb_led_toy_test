...
void setup(void)
{
	...
	set_all_rgb(0, 255, 0, 1);	// when you see GREEN, turn the pot to the minimum setting
	delay(2000);
	set_low_limit(&low_limit);
	set_all_rgb(255, 0, 0, 1);	// when you see RED, turn it to the maximum setting
	delay(2000);
	set_high_limit(&high_limit);
	set_all_rgb(0, 0, 0, 1);	// after about 2 seconds, turn the pot up and down and observe the effect
}
...
void loop(void)
{
	...
	byte led_number =
	    (byte) (constrain
		    (map(adc_value, low_limit, high_limit, 0, 8), 0, 8));

	if (adc_value_prev != adc_value) {
		set_all_rgb(0, 0, 0, 0);	// don't flip the buffers here, just clear the current one
		if (led_number > 0) {
			led_number--;
			for (ctr = 0; ctr <= led_number; ctr++) {
				...
				//set_led_hsv(led,led_number*8,255,led_number*28+32, 0);
				//set_led_hsv(led,220,255,255, 0);
				set_led_hsv(led, led * 32, 255, 255, 0);
			}
		}
		flip_buffers();
	}
