...
/* all of these volatile variables will be set in setup_timer1_ovf() */
...
setup (void)
{
  DDRB |= (_BV (LED0) | _BV (LED1) | _BV (LED2) | _BV (LED3) | _BV (LED4) | _BV (LED5) | _BV (LED6) | _BV (LED7));	// set PORTB as output
      ...
      if (twi_rx_complete_flag == 1)
	{
	  ...
	  switch (twi_rx_buffer[0])
	    ...
	    case 0:
	      ...
	      break;
	    case 1:
	        random_delay_blink_color((COLOR_t)(twi_rx_buffer[1]));
	      break;
            case 3:
              ...
              break;
            case 4:
              disable_timer1_ovf ();		// start PWM mode
              break;
            case 5:
              set_led_rgb(twi_rx_buffer[1],(twi_rx_buffer[2] & 0x04),(twi_rx_buffer[2] & 0x02),(twi_rx_buffer[2] & 0x01));
              break;
            case 6:
...
#ifdef SLAVE
void
twi_slave_handler (int dummy_rcvd_bytes)
{
  uint8_t buff_pos = 0;
...
void
random_delay_blink_color(enum COLOR_t led_color) {
  color_on(led_color);
  PORTB = 0x00;
...
#ifdef DOTCORR
  int8_t dotcorr =
    (int8_t) (pgm_read_byte (&dotcorr_red[led])) * red / brightness_levels;
  uint8_t value;
  if (red + dotcorr < 0)
    {
      value = 0;
...
#ifdef DOTCORR
  int8_t dotcorr =
    (int8_t) (pgm_read_byte (&dotcorr_blue[led])) * blue / brightness_levels;
  uint8_t value;
  if (blue + dotcorr < 0)
    {
      value = 0;
